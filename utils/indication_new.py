...
def CR_stat_indication(trials, fec, fec_time, static_threshold=0.02, AP_delay=3):
    fec_index_0 = {}
    fec_index_led = {}
    fec_index_ap = {}
    fec_index_cr = {}
    ...
    for trial_id in fec:
        try:
            if trial_id not in trials:
                print(f"trial {trial_id} missing in trials dict")
                continue

            trial = trials[trial_id]

            if "LED" not in trial or "AirPuff" not in trial:
                print(f"trial {trial_id} missing LED or AirPuff")
                continue

            led_raw = trial["LED"]
            ap_raw = trial["AirPuff"]

            # Validate types
            led_time = led_raw[0] if isinstance(led_raw, (list, np.ndarray)) else float(led_raw)
            ap_time = ap_raw[0] if isinstance(ap_raw, (list, np.ndarray)) else float(ap_raw)

            time_vec = fec_time.get(trial_id)
            fec_trace = fec.get(trial_id)

            if time_vec is None or fec_trace is None:
                print(f"trial {trial_id} missing fec or time vector")
                continue

            smoothed_trace = smoothed_isi.get(trial_id)
            if smoothed_trace is None:
                print(f"trial {trial_id} missing smoothed fec")
                continue

            fec_time_0[trial_id] = time_vec

            fec_index_0[trial_id] = np.abs(time_vec).argmin()
            fec_index_led[trial_id] = find_index(time_vec, 0.0)
            fec_index_ap[trial_id] = find_index(time_vec, ap_time - led_time + 12)
            fec_index_cr[trial_id] = find_index(time_vec, ap_time - led_time - 50)
            fec_index_bl[trial_id] = find_index(time_vec, -200)

            # Check index ranges
            if fec_index_cr[trial_id] >= fec_index_ap[trial_id] or fec_index_bl[trial_id] >= fec_index_led[trial_id]:
                print(f"trial {trial_id} has invalid CR or baseline interval")
                continue

            CR_interval = fec_trace[fec_index_cr[trial_id]:fec_index_ap[trial_id]]
            ISI_interval = smoothed_trace[fec_index_led[trial_id]:fec_index_cr[trial_id]]
            baseline_segment = fec_trace[fec_index_bl[trial_id]:fec_index_led[trial_id]]

            if len(CR_interval) == 0 or len(ISI_interval) == 0 or len(baseline_segment) == 0:
                print(f"trial {trial_id} has empty interval(s)")
                continue

            sorted_baseline = np.sort(baseline_segment)
            baseline_trimmed = sorted_baseline[:int(0.3 * len(sorted_baseline))]

            # Averages
            CR_interval_avg[trial_id] = np.mean(CR_interval)
            isi_interval_avg[trial_id] = np.mean(ISI_interval)
            base_line_avg[trial_id] = np.mean(baseline_trimmed)

            # Classification
            if CR_interval_avg[trial_id] - base_line_avg[trial_id] > static_threshold:
                CR_stat[trial_id] = 1
                cr_positives.append(trial_id)
            elif np.any(ISI_interval > base_line_avg[trial_id] + static_threshold):
                CR_stat[trial_id] = 2  # poor CR
            else:
                CR_stat[trial_id] = 0  # CR-

            cr_interval_idx[trial_id] = np.array([fec_index_cr[trial_id], fec_index_ap[trial_id]])
            bl_interval_idx[trial_id] = np.array([fec_index_bl[trial_id], fec_index_led[trial_id]])

        except Exception as e:
            print(f"trial {trial_id} has trial type file problem: {type(e).__name__}: {e}")
            continue
