...
def aligning_times(trials, aligning_event="LED"):
    ...
    for id in trials:
        aligned_time = trials[id]["time"][:] - trials[id][aligning_event][0]
        trial_len = len(aligned_time)

        idx_start = np.searchsorted(aligned_time, init_time, side='right')
        idx_end = np.searchsorted(aligned_time, ending_time, side='left')
        idx_led = np.searchsorted(trials[id]["time"][:], trials[id][aligning_event][0], side='right')
        idx_ap = np.searchsorted(trials[id]["time"][:], trials[id]["AirPuff"][0], side='right')

        # Skip invalid index ranges
        if idx_start >= trial_len or idx_end > trial_len or idx_start >= idx_end:
            continue

        init_index[id] = idx_start
        ending_index[id] = idx_end
        led_index[id] = idx_led
        ap_index[id] = idx_ap
        event_diff.append(idx_led - idx_start)
        ending_diff.append(idx_end - idx_led)

    if len(init_index) == 0:
        raise ValueError("No valid trials for alignment!")

    init_incr = min(event_diff)
    ending_incr = min(ending_diff)

    # Final: re-center all trials to have same window
    init_index_0 = {}
    ending_index_0 = {}

    for id in init_index:
        init0 = led_index[id] - init_incr
        end0 = led_index[id] + ending_incr

        # Final safety check: avoid overflow
        if init0 < 0 or end0 > len(trials[id]["time"][:]):
            continue

        init_index_0[id] = init0
        ending_index_0[id] = end0
...
def aligned_dff(trials, trials_id, cr, cr_stat, init_index, ending_index, sample_id):
    ...
    n_rois = len(trials[sample_id_str]["dff"])
    for roi in range(n_rois):
        aligned_dff[roi] = {}

    for trial_id in trials_id:
        trial_id_str = str(trial_id)
        if cr is not None and cr.get(trial_id) != cr_stat:
            continue

        for roi in range(len(trials[trial_id_str]["dff"])):
            dff_value = trials[trial_id_str]["dff"][roi][init_index[trial_id]:ending_index[trial_id]]
            aligned_dff[roi][trial_id] = dff_value

        aligned_time[trial_id] = trials[trial_id_str]["time"][init_index[trial_id]:ending_index[trial_id]] - trials[trial_id_str]["LED"][0]
